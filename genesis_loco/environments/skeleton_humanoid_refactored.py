...
class SkeletonHumanoidEnv:
    ...
    def num_observations(self) -> int:
        """Calculate observation space size (exact same as current)"""
        # Root: 5 (z + quat) + Joints: 22 + Root vel: 6 + Joint vel: 22 = 55
        return 5 + min(22, self.robot.n_dofs) + 6 + min(22, self.robot.n_dofs)
    ...
    def _apply_actions(self, actions: torch.Tensor):
        """Apply torque actions directly to joints using correct Genesis DOF control (from current)"""
    ...
    def _check_termination(self) -> torch.Tensor:
        """Check skeleton-specific termination conditions (exact same as current)"""
        # Default termination: episode length exceeded (from base env)
        done = self.episode_length_buf >= self.max_episode_length
        
        # Height limits (LocoMujoco: 0.8-1.1m) (from skeleton)
        height_violation = (self.root_pos[:, 2] < 0.8) | (self.root_pos[:, 2] > 1.1)
        done = done | height_violation
        
        # Orientation limits (45 degree roll/pitch) (from skeleton)
    ...
    def _get_observations(self) -> torch.Tensor:
        ...
        num_joint_obs = min(22, self.robot.n_dofs)  # LocoMujoco has 22 joint positions
    ...
    def _reset_robot_pose(self, env_ids: torch.Tensor):
        ...
        # Standing position (exact same values)
